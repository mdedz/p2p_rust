...
pub async fn run(port: u16, peer_table: Arc<Mutex<HashMap<String, Peer>>>, uname: &String) -> anyhow::Result<()>{
    ...
    loop {
        ...
        tokio::spawn(async move {
            ...
            let mut peer = Peer::new(addr_str.clone(), socket, &"Stranger".to_string());
            
            peer_table.lock().await.insert(addr_str.clone(), peer.clone());
            send_join(&mut peer, uname_copy).await;
